...
gwas <- fread("../SURVIVAL_MORTALITY_META_20220513_hg191.tbl")
...
gwas_qc_formatted <- gwas_qc %>% 
  mutate(SNP = paste("chr", MarkerName, sep = "")) %>% 
...
hits <- gwas_qc_formatted %>% 
  filter(P < 5.5e-08) #Top GWAS hits will be annotated (and including one nominal assoc in SYT10)
...
hits_annotated <- res.annotated %>% 
  ...
  mutate(STATUS = 0) %>% #Going to code all as STATUS = 0
...
plotting <- gwasToPlot %>%
  ...
  mutate(is_annotate=ifelse(log10P>7.25, "yes", "no")) #adjusted this threshold slightly from 7.30103 to 7.25
...
# Export it
ggsave("./plots/mortality_manhattan.pdf", thisManhattan, width = 8, height = 4, dpi=300, units = "in")
ggsave("./plots/mortality_manhattan.png", thisManhattan, width = 11, height = 3.5, dpi=300, units = "in")
ggsave("./plots/mortality_manhattan.svg", thisManhattan, width = 7, height = 3.5, dpi=300, units = "in")

#For MDS abstract
ggsave("./plots/mortality_manhattan_abstract.png", thisManhattan, width = 7, height = 4, dpi=300, units = "in")
